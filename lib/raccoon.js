...
let Raccoon = function(game_option) {
    ...
    function in_threefold_repetition() {
        ...
        for(let i=0; i < board.history_ply; i++){
            ...
            if (board.current_polyglot_key === board.history[i].current_polyglot_key) count++;
        }
        return count >= 2;
    }
    ...
    function in_draw() {
        ...
        return is_50_move || insufficient_material() || in_threefold_repetition() ;
    }
    ...
    function polyglot_key(){
        ...
        let final_key = 0n, piece = PIECES.EMPTY, poly_piece = 0, offset;
    ...
    function history(verbose) {
        let verbose_move = NO_MOVE;
        let rlt = [];
        for(let i=0; i < board.history_ply; i++){
            verbose_move = board.history[i];
            if(verbose){
                rlt.push(verbose_move.summary);
            }
            else{
                rlt.push(verbose_move.summary.san);
            }
        }
        return rlt;
    }
    ...
    return {
        ...
        move: function (move) {
            ...
            if(typeof move === 'string'){
                ...
                if (maybe_smith !== ""){
                    move_t = smith_to_move(maybe_smith);
                }
                else{
                    move_t = san_to_move(move);
                }
            }
